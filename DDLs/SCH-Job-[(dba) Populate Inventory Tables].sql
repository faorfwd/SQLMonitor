...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(dba) Populate Inventory Tables', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.usp_wrapper_populate_sma_sql_instance', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@os_run_priority=0, 
		@subsystem=N'CmdExec',
		@command=N'sqlcmd -C -E -b -S localhost -H "(dba) Populate Inventory Tables" -d "DBA" -Q "exec dbo.usp_wrapper_populate_sma_sql_instance @send_mail = 1, @verbose = 0, @truncate_log_table = 1;"', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'(dba) Populate Inventory Tables', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
