...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(dba) Run-WhoIsActive', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Capture sp_WhoIsActive result into dbo.WhoIsActive.
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.usp_run_WhoIsActive', 
		...
		@command=N'sqlcmd -C -E -b -S localhost -H "(dba) Run-WhoIsActive" -d DBA -Q "DECLARE @e BIT = 1; SELECT @e = ISNULL(CONVERT(BIT, param_value), 1) FROM dbo.sma_params WHERE param_key = ''email_delivery_enabled''; EXEC dbo.usp_run_WhoIsActive @retention_day = 7, @recipients = ''dba_team@gmail.com'', @email_delivery_enabled = @e;"',
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
