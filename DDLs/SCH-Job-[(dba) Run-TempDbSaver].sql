...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'usp_TempDbSaver', 
		...
		@command=N'sqlcmd -C -E -b -S localhost -H "(dba) Run-TempDbSaver" -d DBA -Q "DECLARE @e BIT = 1; SELECT @e = ISNULL(CONVERT(BIT, param_value), 1) FROM [sqlmonitor_inventory_server].[DBA].dbo.sma_params WHERE param_key = ''email_delivery_enabled''; EXEC dbo.usp_TempDbSaver @email_recipients = ''dba_team@gmail.com'', @data_used_pct_threshold = 80, @data_used_gb_threshold = 500, @threshold_condition = ''or'', @purge_table = 1, @kill_spids = 0, @send_email = 0, @drop_create_table = 0, @verbose = 0, @email_delivery_enabled = @e;"',
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
