...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.usp_collect_wait_stats', 
		@step_id=1, 
		...
		@command=N'sqlcmd -C -E -b -S localhost -H "(dba) Collect-WaitStats" -d DBA -Q "DECLARE @e BIT = 1; SELECT @e = ISNULL(CONVERT(BIT, param_value), 1) FROM [sqlmonitor_inventory_server].[DBA].dbo.sma_params WHERE param_key = ''email_delivery_enabled''; EXEC dbo.usp_collect_wait_stats @recipients = ''dba_team@gmail.com'', @email_delivery_enabled = @e;"',
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
