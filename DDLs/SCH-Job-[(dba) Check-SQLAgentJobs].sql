...
IF EXISTS (SELECT * FROM msdb.dbo.sysjobs_view WHERE name = N'(dba) Check-SQLAgentJobs')
	EXEC msdb.dbo.sp_delete_job @job_name='(dba) Check-SQLAgentJobs', @delete_unused_schedule=1
GO
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(dba) Check-SQLAgentJobs', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Collect SQL Agent Job Stats.
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'usp_check_sql_agent_jobs', 
		@step_id=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'(dba) Check-SQLAgentJobs', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
